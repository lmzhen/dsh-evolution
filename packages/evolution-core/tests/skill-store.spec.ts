...
it('skill create/update/patch/archive are recoverable', async () => {
  const root = await tempRoot('dsh-evo-skills-')
  const lib = new SkillLibrary(root)
...
it('snapshot co-copies usage/suppression sidecars and restore returns them', async () => {
  const root = await tempRoot('dsh-evo-skills-sidecars-')
  const lib = new SkillLibrary(root)
...
it('snapshot co-copies .archive and restore replaces it with the snapshot state', async () => {
  const root = await tempRoot('dsh-evo-skills-archive-snap-')
  const lib = new SkillLibrary(root)
...
it('V26-14: snapshotAll probes live write locks — a locked skill is skipped, recorded, and not copied', async () => {
  const root = await tempRoot('dsh-evo-skills-probe-')
  const lib = new SkillLibrary(root)
...
it('V27 G0.1 (core-a-10): an INCOMPLETE snapshot is refused before the destructive clear', async () => {
  const root = await tempRoot('dsh-evo-skills-skipped-')
  const lib = new SkillLibrary(root)
  await lib.create('kept-skill', USABLE('kept-skill'), 'foreground')
  await lib.create('locked-skill', USABLE('locked-skill'), 'foreground')
...
it('V27 G0.1 (core-a-10): a snapshot that skipped EVERY skill cannot empty the live library', async () => {
  const root = await tempRoot('dsh-evo-skills-allskip-')
...
it('V27 G0.3 (core-a-07): a skill whose description is a YAML block scalar is writable and readable', async () => {
  const root = await tempRoot('dsh-evo-skills-blockscalar-')
  const lib = new SkillLibrary(root)
...
it('V27 G0.4 (core-a-01): a manifest with non-string extras is refused before the tree is touched', async () => {
  const root = await tempRoot('dsh-evo-skills-extras-gate-')
  const lib = new SkillLibrary(root)
  await lib.create('keeper-skill', USABLE('keeper-skill'), 'foreground')
...
it('snapshot extras are manifest-declared and only declared names are read back', async () => {
  const root = await tempRoot('dsh-evo-skills-extras-')
  const lib = new SkillLibrary(root)
  await lib.create('keeper-skill', USABLE('keeper-skill'), 'foreground')
...
it('names normalize at the path choke point so padded aliases cannot fork a skill (P2-5)', async () => {
  const root = await tempRoot('dsh-evo-skills-dir-')
  const lib = new SkillLibrary(root)
...
it('V7-11: a replaceAll beyond the CUMULATIVE fuzzy budget is refused whole (0.3.44)', async () => {
  ...
  const root = await tempRoot('dsh-evo-skills-v711-')
  const lib = new SkillLibrary(root)
...
it('V9-04: consolidate of a MISSING target refuses BEFORE archiving any source (0.3.50)', async () => {
  const root = await tempRoot('dsh-evo-skills-v904-')
  const lib = new SkillLibrary(root)
  await lib.create('src-a', USABLE('src-a'), 'foreground')
...
it('list() reports dot-prefixed protection markers with bundled-hub-pinned precedence (N-1)', async () => {
  const root = await tempRoot('dsh-evo-skills-list-')
  const lib = new SkillLibrary(root)
...
it('same-second re-archives get unique stamped destinations (N-6)', async () => {
  const root = await tempRoot('dsh-evo-skills-arc-n6-')
  const lib = new SkillLibrary(root)
  const paths: string[] = []
...
it('create normalizes unquoted YAML-unsafe frontmatter at the write point (0.3.11)', async () => {
  const root = await tempRoot('dsh-evo-skills-norm-create-')
  const lib = new SkillLibrary(root)
...
it('create quotes quote/backslash values via single-quote fallback — no catch-22 (0.3.11 fix)', async () => {
  const root = await tempRoot('dsh-evo-skills-norm-quote-')
  const lib = new SkillLibrary(root)
...
it('update and patch normalize frontmatter the same way (0.3.11)', async () => {
  const root = await tempRoot('dsh-evo-skills-norm-upd-')
  const lib = new SkillLibrary(root)
...
it('E-68: an old===new patch is a noop — no write, no audit, no mutation event (0.3.18)', async () => {
  const root = await tempRoot('dsh-evo-skills-noop-')
  const events: string[] = []
...
it('F-318 (②): update with a byte-equivalent content is a noop — no write, no audit (E-68)', async () => {
  const root = await tempRoot('dsh-evo-skills-upd-noop-')
  const lib = new SkillLibrary(root)
  const created = await lib.create('python-testing', SKILL, 'background_review')
  expect(created.ok).toBe(true)
  const before = await lib.read('python-testing')
...
it('E-69: an archived skill cannot absorb into itself (0.3.18)', async () => {
  const root = await tempRoot('dsh-evo-skills-selfabs-')
  const lib = new SkillLibrary(root)
  const created = await lib.create('python-testing', SKILL, 'background_review')
  expect(created.ok).toBe(true)
...
it('F-316: a drop-in root dot-file created AFTER the snapshot is cleaned on restore, while the audit sidecar survives', async () => {
  const root = await tempRoot('dsh-evo-skills-dotrestore-')
  const lib = new SkillLibrary(root)
...
it('V6-17: a non-exact anchor past the fuzzy budget is refused with an honest message (0.3.37)', async () => {
  const root = await tempRoot('dsh-evo-skills-fuzzyguard-')
  const lib = new SkillLibrary(root)
  await lib.create('python-testing', SKILL, 'foreground')
...
it('A1-17 (v18): a failed marker probe reports protectionUnknown, never "unprotected"', async () => {
  const root = await tempRoot('dsh-evo-skills-unknown-')
...
it('A1-20 (v18): restoring onto a directory without SKILL.md names the real obstacle', async () => {
  const root = await tempRoot('dsh-evo-skills-restore-dir-')
  const lib = new SkillLibrary(root)
...
it('计划 B-4 (v18): SKILL_NAME_RE matches the upstream shape', async () => {
  ...
  const root = await tempRoot('dsh-evo-skills-name-')
  const lib = new SkillLibrary(root)
...
it('E-11 (v18): list() publishes the frontmatter whenToUse routing hint', async () => {
  const root = await tempRoot('dsh-evo-skills-when-')
  const lib = new SkillLibrary(root)
...
it('V27 G2.1: list() publishes the strict catalog value, and the audit verdict agrees', async () => {
  const root = await tempRoot('dsh-evo-skills-strict-')
  const lib = new SkillLibrary(root)
...
it('V27 G8.4: an archived skill whose SKILL.md is empty is still restorable by name', async () => {
  ...
  const root = await tempRoot('dsh-evo-skills-archname-')
  const lib = new SkillLibrary(root)
...
it('A1-15 (v18): a post-commit dir-fsync failure still audits and reports a durability warning', async () => {
  const root = await tempRoot('dsh-evo-skills-durability-')
...
it('P2-2 (v19): create and setPinned tolerate a post-commit fsync failure too', async () => {
  const root = await tempRoot('dsh-evo-skills-durability2-')
  const base = nodeEvolutionIo()
...
it('P1-2 (v19): a healthy empty-tree snapshot WITH sidecars restores (guard reads manifest.sidecars)', async () => {
  const root = await tempRoot('dsh-evo-snap-sidecar-')
  const lib = new SkillLibrary(root)
...
it('P1-2 (v19): a truncated manifest (declares no skills, directory has UNDECLARED entries) is still refused', async () => {
  const root = await tempRoot('dsh-evo-snap-truncated-')
  const lib = new SkillLibrary(root)
...
it('P2-4 (v19): a non-string manifest entry is refused structurally, not with a TypeError', async () => {
  const root = await tempRoot('dsh-evo-snap-nonstring-')
  const lib = new SkillLibrary(root)
...
it('v28 G1.1 (EVO-IO-02): update on a ghost directory removes it instead of blocking restore forever', async () => {
  const root = await tempRoot('dsh-evo-skills-ghost-')
  const lib = new SkillLibrary(root)
  await lib.create('ghost-skill', SKILL.replace('python-testing', 'ghost-skill'), 'foreground')
...
it('v28 G1.1 (EVO-IO-02): the ghost cleanup never takes a directory with real content', async () => {
  const root = await tempRoot('dsh-evo-skills-ghost2-')
  const lib = new SkillLibrary(root)
...
it('v28 G2.5 (CORE-SK-03): both refusals pre-clear report an UNCHANGED tree, not "Rescue manually"', async () => {
  const root = await tempRoot('dsh-evo-skills-restore3-')
  const lib = new SkillLibrary(root)
...
it('v29 LIST-01: a directory squatting on SKILL.md skips the entry instead of killing the whole listing', async () => {
  const root = await tempRoot('dsh-evo-skills-listeisdir-')
  const lib = new SkillLibrary(root)
...
it('v30 REG-01: a non-EISDIR read failure (EACCES) still fails list() loud — no silent partial tree', async () => {
  const root = await tempRoot('dsh-evo-skills-acces-')
...
it('v35 C9: a matching stage-time anchor lets the update through', async () => {
  const root = await tempRoot('dsh-c9-anchor-ok-')
  const lib = new SkillLibrary(root)
  const original = SKILL.replace('python-testing', 'anchor-skill')
  await lib.create('anchor-skill', original, 'foreground')
  const staged = contentHash(await lib.read('anchor-skill') ?? '')
  const updated = await lib.update('anchor-skill', original.replace('Run tests with pytest.', 'Run tests with pytest -q.'), 'foreground', { sha256: staged })
  expect(updated.ok).toBe(true)
  expect(await lib.read('anchor-skill')).toContain('pytest -q')
})

it('v35 C9: a drifted anchor refuses the update and leaves the bytes untouched', async () => {
  const root = await tempRoot('dsh-c9-anchor-drift-')
  const lib = new SkillLibrary(root)
  const original = SKILL.replace('python-testing', 'drift-skill')
  await lib.create('drift-skill', original, 'foreground')
  const staged = contentHash(await lib.read('drift-skill') ?? '')
  // A concurrent writer lands after staging, before the plan's write.
  await lib.update('drift-skill', original.replace('Run tests with pytest.', 'A concurrent edit.'), 'foreground')
  const landed = await lib.read('drift-skill')
  const refused = await lib.update('drift-skill', original.replace('Run tests with pytest.', 'The stale plan.'), 'foreground', { sha256: staged })
  expect(refused.ok).toBe(false)
  expect(refused.stale).toBe(true)
  expect(refused.anchor).toBe('drift')
  // The concurrent writer's bytes survived: the stale plan did not overwrite them.
  expect(await lib.read('drift-skill')).toBe(landed)
})

it('v35 C9: a vanished target reports the anchor verdict, and unanchored callers keep the not-found report', async () => {
  const root = await tempRoot('dsh-c9-anchor-gone-')
  const lib = new SkillLibrary(root)
  const content = SKILL.replace('python-testing', 'never-created')
  const refused = await lib.update('never-created', content, 'foreground', { sha256: 'deadbeef' })
  expect(refused.ok).toBe(false)
  expect(refused.stale).toBe(true)
  expect(refused.anchor).toBe('missing')
  const plain = await lib.update('never-created', content, 'foreground')
  expect(plain.ok).toBe(false)
  expect(plain.stale).toBeUndefined()
  expect(plain.message).toBe('Skill "never-created" not found.')
})

it('v35 C9: an absent anchor creates a support file and refuses one that already exists', async () => {
  const root = await tempRoot('dsh-c9-anchor-absent-')
  const lib = new SkillLibrary(root)
  await lib.create('support-skill', SKILL.replace('python-testing', 'support-skill'), 'foreground')
  const created = await lib.writeSupportFile('support-skill', 'references/notes.md', 'first\n', 'foreground', { absent: true })
  expect(created.ok).toBe(true)
  const refused = await lib.writeSupportFile('support-skill', 'references/notes.md', 'second\n', 'foreground', { absent: true })
  expect(refused.ok).toBe(false)
  expect(refused.stale).toBe(true)
  expect(refused.anchor).toBe('drift')
  expect(await lib.readSupportFile('support-skill', 'references/notes.md')).toBe('first\n')
})

it('v35 C9: a support-file remove honours its anchor and keeps drifted bytes', async () => {
  const root = await tempRoot('dsh-c9-anchor-remove-')
  const lib = new SkillLibrary(root)
  await lib.create('remove-skill', SKILL.replace('python-testing', 'remove-skill'), 'foreground')
  await lib.writeSupportFile('remove-skill', 'references/gone.md', 'staged\n', 'foreground')
  const staged = contentHash(await lib.readSupportFile('remove-skill', 'references/gone.md') ?? '')
  await lib.writeSupportFile('remove-skill', 'references/gone.md', 'changed since staging\n', 'foreground')
  const refused = await lib.removeSupportFile('remove-skill', 'references/gone.md', 'foreground', { sha256: staged })
  expect(refused.ok).toBe(false)
  expect(refused.stale).toBe(true)
  expect(await lib.readSupportFile('remove-skill', 'references/gone.md')).toBe('changed since staging\n')
  // The CURRENT hash lets the same remove through.
  const current = contentHash(await lib.readSupportFile('remove-skill', 'references/gone.md') ?? '')
  expect((await lib.removeSupportFile('remove-skill', 'references/gone.md', 'foreground', { sha256: current })).ok).toBe(true)
})
