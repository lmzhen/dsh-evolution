...
import { evolutionHome, softCostUnitsFor, DEFAULT_CURATOR_INTERVAL_HOURS, DEFAULT_HEALTH_THRESHOLDS, DEFAULT_MIN_IDLE_HOURS, DEFAULT_STALE_AFTER_DAYS, DEFAULT_ARCHIVE_AFTER_DAYS, clampedNumber } from '@deepseek-ai/dsh-evolution-core'
...
export class EvolutionCurator extends Service {
  ...
  async healthView(): Promise<Array<{ name: string; verdict: SkillHealthVerdict; reasons: string[] }>> {
    const thresholds: SkillHealthThresholds = {
      ...
      // Derived from the configured char ceiling instead of adding a fourth
      // config field: one conversion, no second number to keep in step.
      softBodyCostUnits: softCostUnitsFor(this.healthSoftBodyChars),
    }
