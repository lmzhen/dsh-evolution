...
没有它：Agent 每次会话都对你的纠正无记忆，已经摸索过的流程会被重新推导。装上后：纠正跨会话保留，发现的工作流沉淀为可复用技能，技能库按计划被评审与治理——统统在审批策略之下。

默认保守：后台评审只改写本会话读过的技能；pinned 技能对它只读。分阶段审批（后台写入先经你批准/拒绝）距一行配置；每次变更都有日志。
...
**典型用途：**
- **长期个人助手。** 纠正一次——"用绝对路径""装工具前先问"——后续会话即生效；它摸索出的流程（备份配方、技术栈的坑）沉淀为技能，下次直接复用而非重新推导。
- **多会话共享技能库。** 多个会话共享一个 evolution host：贡献者把可复用工作流做成技能，评审收进共享库，后续会话都能找到。
- **无人值守自动化。** 定时任务不挂模型工具；用量遥测与 curator 在后台保持库健康，日常会话保持精简。
- **审计与治理。** 查看 Agent 学了什么、准备改什么；批准/拒绝分阶段写入；快照回滚；全程有迹可查。
