...
29 个可组合的 Cordis 插件，给一套 DeepSeek Harness 装上自进化层：Agent 会回顾自己的对话，维护**记忆**、提出并修补**技能**，并让这个技能库随时间被治理。模型只能写记忆和技能——策略、提示词、路由、状态和审计历史是它永远碰不到的控制面数据。

它不是模型，不是托管服务，也没有自己的 GUI：它加的是模型工具、斜杠命令，以及你自己 `$DSH_HOME` 下的文件。
...
M 编号在**本文件**定义；安装**形态**及各形态在平台线上的状态单源在 `packages/INSTALL.md`。
...
| 家族版本 | `0.3.83`（npm `latest`；各形态状态见 `packages/INSTALL.md`） |
| Node | 22.19+ 或 24+（`engines`） |

预发布 range 只认**一个**锚点，而不是一族：`^0.1.5-rc.2` 会拒绝更晚的预发布后继版，但接受稳定版 `0.1.5`。更早的预发布线在依赖解析阶段就失败——这是支持窗口，不是 bug。
...
有两个默认值值得先知道。**评审在会话内跑**（`reviewMode: 'inject'`，0.3.74 起就是默认）——不 spawn 任何东西，因此没有单独的评审账本；想要干净的父上下文或专用模型，`subagent` 是显式 opt-in。**跨会话消费者按会话生效：** review 与用量遥测只有在某个会话能解析出家族的模型工具时才作用于它——M1 下每个会话都成立，host-only 安装（M3）则永远不成立，插件会为此报告一次，而不是静默失败。
...
- `/evolution mutations` —— 写入日志；`/graph` —— 把技能与记忆画成图谱。

完整的命令面只渲染一次、住在 `packages/README.md`（§Command reference）；环境变量与字段级旋钮的完整参考也在那里。
...
| threatStrictness | strict / 豁免清单 | `threatExemptLabels`（按配置点声明；清单单源在 `evolution-threat/README.md`） |
...
**还什么都没写。** 第一次评审要等间隔窗口（`memoryInterval` / `skillInterval`，默认 10 轮）和 substantive 门——一行字的会话，故意不值得评审。
...
| `E-306` | 该部署对前台写入做 stage，但这条命令没法通过技能 runner 重放 | 直接 approve-and-execute，或有意设 `stageForeground: false` |
...
回路停了，数据不会：记忆、技能、状态、报告和审批历史都留在 `$DSH_HOME` 下，重装时会再次被接管。
...
4. **逐技能调用的 frontmatter** —— 影子 provider 必须继续解析和上游一样的键；它对旧键的处理口径单源在 `evolution-skill-catalog/README.md`。
