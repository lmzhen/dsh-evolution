...
> **口径更正（本轮实测）**：`packages/README.md` 与审计档都写「20 键／22 条」，实测本轮开工前是 **22 键／22 条**（`E-301`–`E-316`，每个键一条原文），本轮加 `E-318` 后为 **23 键／23 条**，范围 `E-301`–`E-318`；两处文档均按实测改正（"measured" 的计数写错也是错，一并纠正）。
...
> **验证**：新 spec `tool-skill-manage/tests/write-gates.spec.ts` 17 条（未读被拒／读过放行／失败的读不算／前台豁免／日志不可读放行＋只警告一次／纯判定先于读判定／准入先于审批缝不产生 pending／执行点仍拒存下来的坏记录；确认：问一次且选项正确／拒绝即无写入／裸删除问、吸收删除不问／子代理与回放都不问／缺参先于提问／无服务放行＋只警告一次／`CALLER_NOT_LIVE` 回退到注册表的活 root／无活 root 放行＋警告）。
