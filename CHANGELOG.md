...
## 0.9.0 (minor) — 用词改版：界面／命令／文档说人话（「策展」等内部词退场）
...
> **行为变化（只影响技能写入的工具路径）**：非前台（子代理／审查注入）去写一个本会话**没读过**的技能，过去无人拦，现在**拒绝并回 `E-318`**、提示先用 `skill` 工具读；只算**没失败**的读（v32 REV-06(a) 的旧口径不变）。前台操作者自己的会话照旧豁免；会话日志在这个组合里读不到时**放行并只警告一次**（软探针口径，与跨源检查一致）。审查的计划路径口径不变（仍是剔除未读 op，只是改用同一份判定）。
> **口径更正（本轮实测）**：`packages/README.md` 与审计档都写「20 键／22 条」，实测本轮开工前是 **22 键／22 条**（`E-301`–`E-316`，每个键一条原文），本轮加 `E-318` 后为 **23 键／23 条**，范围 `E-301`–`E-318`；两处文档均按实测改正（"measured" 的计数写错也是错，一并纠正）；B3 再加 `E-317` 后为 **24 键／24 条**。
