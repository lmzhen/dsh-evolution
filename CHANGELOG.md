...
> **验证**：新 spec `tool-skill-manage/tests/write-gates.spec.ts` 9 条（未读被拒／读过放行／失败的读不算／前台豁免／日志不可读放行＋只警告一次／纯判定先于读判定／准入先于审批缝不产生 pending／执行点仍拒存下来的坏记录）。
